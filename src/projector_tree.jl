...
"""
function count_site_projections(
    parttt::PartitionedTT, all_sites::AbstractVector{<:Index}
)::Dict{Index,Int}
...
function build_projector_tree(
    ...
    return _build_tree_recursive(
        parttt, sorted_sites, Dict{Projector,SubDomainTT}(parttt.data)
    )
end
...
function find_in_tree(
    tree::ProjectorTreeNode, target_projector::Projector, all_sites::AbstractVector{<:Index}
)::Union{Nothing,SubDomainTT}
